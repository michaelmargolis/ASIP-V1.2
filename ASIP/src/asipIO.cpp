...
const char * modeStr[] = {"UNALLOCATED", "INPUT", "INPUT_PULLUP", "OUTPUT", "ANALOG", "PWM","INVALID","OTHER SERVICE", "RESERVED"}; 
#endif 
  ...
  if (strictPinMode) {
    ...
    for ( byte pin = 0; pin < TOTAL_PINCOUNT; pin++) {
      if ( asip.getPinMode(pin) < RESERVED_MODE) {  
        // here if not reserved 
        verbose_printf("pin %d is not reserved\n", pin);
        if (IS_PIN_ANALOG(pin)) {
          // if analog capable pin is also a digital pin then turn off pullups
