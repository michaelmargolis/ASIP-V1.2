...
public:
  ...
  virtual void begin(byte pin); // you can use this terse version when the service has only on pin
  virtual void begin(byte nbrElements, const pinArray_t pins[]);  // you can use this version when there is one pin per element
  virtual void begin(byte nbrElements, byte pinCount, const pinArray_t pins[]); // specify total number of pins
  virtual void begin(byte nbrElements, byte pinCount, const pinArray_t pins[],const byte secondaryPinCount, const pinArray_t secondaryPins[]); 
...
protected:
   ...
   byte singlePin;             // storage location for begin method with single pin instead of pin array  
